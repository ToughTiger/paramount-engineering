...
import { generateBlogPostContent } from '../server/services/geminiService';
import { ImageUpload } from './ImageUpload';
import { getCategoryName } from "../utils/normalize";
import { Category, Tag } from '../types';
type AdminView = 'blog' | 'portfolio' | 'categories' | 'hero' | 'about' | 'settings';



// --- Blog Management Component (inside AdminPanel.tsx) ---
const ManageBlog = () => {
...
const UNCATEGORIZED: Category = { id: 0, name: 'Uncategorized' };
  ...
  const emptyFormState: Omit<BlogPost, 'id' | 'slug' | 'date'> = {
    title: '',
    author: 'Admin',
    imageUrl: '',
    category: categories[0] ?? UNCATEGORIZED,  // Category object
    tags: [] as Tag[],                         // Tag[]
    content: '',
  };
  ...
  // When categories load later, upgrade from UNCATEGORIZED to first real category
  useEffect(() => {
  ...
  // Generic handler for text inputs / textarea
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
    setFormData(prev => ({ ...prev, [name]: value }));
  };

  // Dedicated handler for category (sets a Category object, not an id)
  ...
  // Toggle Tag objects by id
  const handleTagToggle = (tagId: number) => {
    setFormData(prev => {
      const exists = prev.tags.some(t => t.id === tagId);
      const nextTags = exists
        ? prev.tags.filter(t => t.id !== tagId)
        : [...prev.tags, (tags.find(t => t.id === tagId) as Tag)].filter(Boolean);
    ...
    if (editingPost) {
      const updatedPost: BlogPost = {
        ...
        slug: formData.title
          ...
          .replace(/\s+/g, '-')
          .replace(/[^\w-]+/g, ''),
        // date remains the server value; your context update function will send dateISO if needed
      };
      updateBlogPost(updatedPost);
    } else {
      // addBlogPost expects Omit<BlogPost, 'id' | 'slug' | 'date'> with Category & Tag[]
  ...
  const handleGenerateContent = async () => {
    ...
    setFormData(prev => ({ ...prev, title: aiTopic, content }));
    setIsGenerating(false);
  };

  // simple inline getter now that post.category is an object
  const categoryNameOf = (c: Category | undefined) => c?.name ?? 'Uncategorized';
          ...
          {blogPosts.map(post => (
            <div key={post.id} className="border rounded-lg overflow-hidden shadow">
              ...
              <div className="p-2 bg-gray-50 border-t flex justify-end gap-2">
                <button onClick={() => openModal(post)} className="text-blue-500 hover:text-blue-700 p-1">
                  <EditIcon />
                </button>
                <button onClick={() => deleteBlogPost(post.id)} className="text-red-500 hover:text-red-700 p-1">
                  <DeleteIcon />
                </button>
              </div>
            </div>
                        ...
                        return (
                          <button
                            ...
                            className={`px-3 py-1 text-sm rounded-full ${
                              selected ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-700'
                            }`}
                          >
                            {tag.name}
                          </button>
